...
export class ReservationServiceImpl implements ReservationService {
  ...
  getReservationByUserId(userId: string, option: { pagination: PaginationRequest<Reservation>; filter: Filter<Reservation>; }): Promise<Pageable<Reservation>> {
  ...
  async getReservationByRestaurantId(restaurantId: string, option: { pagination: PaginationRequest<Reservation>; filter: Filter<Reservation>; }): Promise<Pageable<Reservation>> {
    return this.reservationRepository.findMany({
  ...
  async createReservation(reservation: ExternalReservationCreateDto): Promise<Reservation> {
    return this.reservationRepository.createOne(reservation.toEntity());
  }

  async createExternalReservation(reservation: ExternalReservationCreateDto): Promise<Reservation> {
    return this.reservationRepository.createOne(reservation.toEntity());
  }
  ...
  async updateReservationStatus(reservationId: string, status: ReservationStatus): Promise<Reservation> {
    ...
    if (reservation.status !== ReservationStatus.PENDING) throw new Error('Reservation status cannot be updated');

    reservation.status = status;
    return this.reservationRepository.updateOne(reservationId, reservation);
  }

  async updateReservation(reservation: Partial<Reservation>): Promise<Reservation> {
    const originalReservation = await this.reservationRepository.findOne(reservation.id);
    if (!originalReservation) throw new Error('Reservation not found');
    if (originalReservation.status !== ReservationStatus.PENDING) throw new Error('Reservation status cannot be updated');
