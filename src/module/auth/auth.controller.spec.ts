...
describe("AuthController", () => {
    ...
    describe("login",()=>{
        ...
        it("should return an exception if email or password is not in body ", async ()=>{
            ...
            expect(authController.login(mockResponse,loginDto)).rejects.toThrow(new HttpException("Email and password are required", 400));
        });
        ...
        it("should not return an exception if email and password are in body", async ()=>{
            ...
            expect(authController.login(mockResponse,loginDto)).rejects.not.toThrow(new HttpException("Email and password are required", 400));
        });
        ...
        it("should return a forbidden if accessToken or refreshToken are not created", async ()=>{
            ...
            await expect(authController.login(mockResponse,loginDto)).rejects.toThrow(ForbiddenException);
        ...
        it("should return a forbidden if an error occured", async ()=>{
            const loginDto: LoginDto = {
                email: "validemail@gmail.com",
                password: "password"
            };
            ...
            await expect(authController.login(mockResponse,loginDto)).rejects.toThrow(new ForbiddenException("Error with tokens"));
        ...
        it("should return an accessToken if everything is ok", async ()=>{
            ...
            // on vérifie que la fonction retourne bien un token et que c'est bien le token généré dans le mock juste au dessus
                expect(authController.login(mockResponse,loginDto)).resolves.toEqual({accessToken: "validToken"});
        });
    ...
    describe("logout",()=>{
        it("should clear the cookie", async ()=>{
            ...
            expect(authController.logout(mockResponse)).resolves.toEqual({message: "Logout success"});
        });
    });
    ...
    describe("validate",()=>{
        ...
        it("should return an exception if there is not token in response", async ()=>{
            ...
            expect(authController.refresh(mockRequest,mockResponse)).rejects.toThrow(BadRequestException);
        });
        ...
        it("should return an exception if the token generated is not valid", async ()=>{
            ...
            expect(authController.refresh(mockRequest,mockResponse)).rejects.toThrow(new ForbiddenException('Invalid refresh token'));
        })
        ...
        it("should return an accessToken if everything is ok", async ()=>{
            ...
            expect(authController.refresh(mockRequest,mockResponse)).resolves.toEqual({accessToken: "valid"});
        });
    });
